...
<head>
    ...
    <script type="text/javascript">
        ...
        function clearCostNum(){
            addCookie('navigationCostId',null,{path:"/"});
                            ...
                            <ul class="nav navbar-nav navbar-left">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-primary">
                                        ...
                                        <#list cost as cost>
                                            <li>
                                                <a href="${ctx!}/admin/navigation/list.do?costId=${(cost.id)!}">${(cost.name)!}</a>
                                            </li>
                                        </#list>
                                    </ul>
                                </li>
					...
					<table id="example-3" class="table table-striped table-bordered" cellspacing="0" width="100%">
						...
						<#list navigations as navigation>
							<tr>
								...
								<td><a href="${ctx!}${(navigation.url)!}" target="_blank">${(navigation.url)!}</a></td>
								<td>
								   <#if navigation.type?? && navigation.type==1>
								        上导航
								   <#elseif navigation.type?? && navigation.type==2> 
								       下导航
								   </#if>
								</td>
								<td>${(navigation.pname)!}</td>
								<#if (cost?size>1)>
						           <td>${(navigation.cost.name)!}</td>
						        </#if>
						    <!--<td>${(navigation.orderid)!}</td>-->
								<td>${((navigation.isShow)??&&(navigation.isShow=1))?string("显示","不显示")}</td>
								<td>
									<a href="${ctx!}/admin/navigation/update.do?id=${(navigation.id)!}" class="btn btn-secondary btn-sm btn-icon icon-left">
										修改
									</a>
									<a href="${ctx!}/admin/navigation/delete.do?id=${(navigation.id)!}" class="btn btn-danger btn-sm btn-icon icon-left" onClick="return confirm('确定要删除?');">
										删除
									</a>
								</td>
							</tr>	
