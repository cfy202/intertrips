...
<script type="text/javascript">
	...
	function selectCost(){
		...
		$.ajax({
			type: "POST",
			url: "${ctx!}/admin/attraction/addbycost.do",
			data: "conum=" + conum,
	...
	function selectid(obj){
		...
		if(im==true){
//			alert(imgid);
			$("#SELECTIMG").append("<input type=\"hidden\" class=\"imginput\" name=\"imageid\" value=\"" + imgid + "\" id=\"" + imgid + "\"/>");
			$("#SELECTIMG").append("<input type=\"hidden\" class=\"" + imgid + "\" name=\"imageurl\" value=\"" + url + "\"/>");
		}else{
			$("#"+imgid).remove();
			$("."+imgid).remove();
		}
...
function showImages(pageNow){
	...
	jQuery(document).ready(function($) {
		$("form#attrForm").validate({
			rules: {
				name: {
					required: true,
					remote: {
						type: "post",
						url: "${ctx!}/admin/attraction/isExist.do",
						data: {
							name: function() {
								return $("#name").val();
							},
				...
				<div class="col-sm-12">
					...
					<div class="panel panel-default">
						<div class="panel-body">
							...
							<form role="form" class="form-horizontal" id="attrForm" action="${ctx!}/admin/attraction/save.do" method="post">
									...
									<div class="col-sm-10">
										<select class="form-control" name="level" id="level">
											<#list attractionLevel as attractionLevel>
											    <#if attractionLevel.level??&&attractionLevel.level=5>
												   <option value="${(attractionLevel.level)!}" selected="selected">${(attractionLevel.description)!}</option>
												   <#else>
												    <option value="${(attractionLevel.level)!}">${(attractionLevel.description)!}</option>
												</#if>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" name="namepy" value="" class="form-control" placeholder="例如a,b,c"/>
									</div>
								</div>
								<div class="form-group-separator"></div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" name="sort" value="${((attraction.sort)?c)!}" class="form-control" id="SORT" placeholder="">
									</div>
								</div> 
								...
								<div class="form-group">
									<div class="col-sm-10" align="center">
										...
										<input name="id" type="hidden" id="id" value="${(attraction.id)!}">
										<input type="submit" name="button" id="button" value="提交" class="btn btn-secondary btn-sm btn-icon icon-left">
										<input type="button" name="" value="取消" onClick="location.href='${ctx!}/admin/attraction/list.do';" class="btn btn-secondary btn-sm btn-icon icon-left">
									</div>								
								</div>     		
							</form>
						</div>
