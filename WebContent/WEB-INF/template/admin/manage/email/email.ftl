						...
						<tbody>
							<#list emailList as emailList>
								<tr>
									...
									<td>${(emailList.addTimeString)!}</td>
									<td>
										<#if emailList.status?? && emailList.status=1>
										订阅中
										<#else>
										已取消
										</#if>
									...
									<td>${(emailList.ip)!}</td>
									<td>${(emailList.costName)!}</td>
									<td>
										<a href="${ctx!}/admin/email/delete.do?id=${emailList.id}" class="btn btn-danger btn-sm btn-icon icon-left" onClick="return confirm('确定要删除?');">
											删除
										</a>																		
									</td>
								</tr>
