 ...
 // 获取Cookie
  ...
  // 添加Cookie
	...
	var clearAllParam = function(){
		...
		addCookie('FopageNow',null,{path:"/"});
		addCookie('FopageSize',null,{path:"/"});
		addCookie('Fosearch',null,{path:"/"});
		addCookie('foodIndexShow',null,{path:"/"});
		addCookie('foodIsshow',null,{path:"/"});
		addCookie('foodIshot',null,{path:"/"});
		$("#searchForm").find(".searchInput").val('');
		$("#pageNowInput").val('');
		window.location.href="${ctx!}/admin/food/list.do";
	}
	//清除cookie中存在的销售中心
	...
	function clearIsShow(status){
	    if(status==1){
	        ...
	        window.location.href="${ctx!}/admin/food/list.do?isshow=1";
	    } else if(status==2){
	        addCookie('tourIndexShow',null,{path:"/"});
	     	addCookie('tourIsshow',null,{path:"/"});
	        window.location.href="${ctx!}/admin/food/list.do?ishot=1";
	    } else if(status==3){
	        addCookie('tourIshot',null,{path:"/"});  
	        addCookie('tourIsshow',null,{path:"/"});
	        window.location.href="${ctx!}/admin/food/list.do?indexShow=1";
	    } else{
		    addCookie('tourIndexShow',null,{path:"/"});
			addCookie('tourIsshow',null,{path:"/"});
			addCookie('tourIshot',null,{path:"/"});
...
<script type="text/javascript">
		...
		function goPage(pageNow){
		    window.location.href="${ctx!}/admin/food/list.do?FopageNow="+pageNow;
		}
</script>
</head>
		...
		<div class="main-content">
			...
			<div class="page-title">
				...
				<div class="breadcrumb-env">
					<ol class="breadcrumb bc-1">
						...
						<li>
							<a href="${ctx!}/admin/food/list.do">美食管理</a>
						</li>
						<li class="active">
							<strong>美食列表</strong>
						</li>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<button class="btn btn-turquoise" onclick="window.location.href='${ctx!}/admin/food/add.do'">添加美食</button>
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-2">
					<form class="navbar-form navbar-left" action="${ctx!}/admin/food/list.do" method="POST" role="search">
						<div class="form-group">
							<input type="text"  class="form-control" onFocus="realse()" placeholder="美食名称、编号、城市" id="tsearch" value="${Hsearch!}" name="Hsearch">
					...
					<ul class="nav navbar-nav navbar-left">
						<li class="dropdown">
							...
							<ul class="dropdown-menu dropdown-primary">
							  ...
							  <#list costlists as costlists>
							    <li>
									<a href="${ctx!}/admin/food/list.do?costId=${(costlists.id)!}">${(costlists.name)!}</a>
								</li>
							  </#list>
							</ul>
						</li>
					...
					<ul class="nav navbar-nav navbar-right">
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu dropdown-primary">
							   <li>
									<a href="${ctx!}/admin/food/list.do?pageSize=10">10</a>
								</li>
							    <li>
									<a href="${ctx!}/admin/food/list.do?pageSize=20">20</a>
								</li>
							    <li>
									<a href="${ctx!}/admin/food/list.do?pageSize=50">50</a>
								</li>
								<li>
									<a href="${ctx!}/admin/food/list.do?pageSize=100">100</a>
								</li>
								<li>
									<a href="${ctx!}/admin/food/list.do?pageSize=${(page.totalCount)?c}">全部</a>
								</li>
							</ul>
						</li>
					</ul>
			...
			<div class="panel-body">
					...
					<form role="forl" action="${ctx!}/admin/visa/create.do" id="myfrom" method="post">
						<table id="example-3" class="table table-striped table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
							  <#list foodlist as foodlist>
							   <tr>
							       <#if costNow??&&costNow=costIdo>
							         <th class="no-sorting">
								      <#if (isCreate?? && isCreate?index_of("${(foodlist.id)!}")!=-1)>
								        <input type="checkbox" name = "tids" value="${(foodlist.id)!}" class="cbr cbr-success">
								        <#else> 
								        <input type="checkbox" name = "tids" value="${(foodlist.id)!}" class="cbr cbr-red">
								      </#if>
									...
									<#else>	
									 <#if admin.username??&&admin.username=='admin'>
										  ...
										  <th class="no-sorting">
										   <input type="checkbox" name = "tids" value="${(foodlist.id)!}">
										  </th>
									 </#if>
								   ...
								   <#if costNow??&&costNow=costIdo>
									...
									<#if foodlist.productTagList?has_content>
									    <#list foodlist.productTagList as tagnames>
									...
									<td>
									   <div id="gallery">
									    <#if foodlist.productProductid.imageurl?has_content>
										    <#list foodlist.productProductid.imageurl?split(",") as urls>
										     <#if urls_index=0>
									...
									<td>${(foodlist.productProductid.code)!}</td>
									<td>${(foodlist.productProductid.minprice)!}</td>
									<td>${(foodlist.foodcn)!}</td>
									<td>${(foodlist.foodstar)!}</td>
									<td>${(foodlist.city)!}</td>
								  <#if costNow??&&costNow=costIdo>
									      <td>
										    <div class="btn-group" width="270">
										              
										                  <#if (hott?? && hott?index_of("${(foodlist.id)!}")!=-1)>
														    <button type="button" id="ishot${(foodlist.id)!}" style="width:60px;"
														    onclick="updateIshot('${(foodlist.id)!}','${(foodlist.productProductid.pagePageid.id)!}');" class="btn btn-red btn-sm">热推</button>
															 <#else>
															<button type="button" id="ishot${(foodlist.id)!}" style="width:60px;"
														    onclick="updateIshot('${(foodlist.id)!}','${(foodlist.productProductid.pagePageid.id)!}');" class="btn btn-red btn-sm">不热推</button>
														</#if>
														 <#if (show?? && show?index_of("${(foodlist.id)!}")!=-1)>
														    <button type="button" id="isshow${(foodlist.id)!}" style="width:60px;"
														    onclick="updateIsshow('${(foodlist.id)!}','${(foodlist.productProductid.pagePageid.id)!}');" class="btn btn-blue btn-sm">显示</button>
															 <#else>
															<button type="button" id="isshow${(foodlist.id)!}" style="width:60px;"
														    onclick="updateIsshow('${(foodlist.id)!}','${(foodlist.productProductid.pagePageid.id)!}');" class="btn btn-blue btn-sm">不显示</button>
														</#if>
														 <#if (index?? && index?index_of("${(foodlist.id)!}")!=-1)>
														    <button type="button" id="isindexshow${(foodlist.id)!}" style="width:80px;"
														    onclick="updateIsindexshow('${(foodlist.id)!}','${(foodlist.productProductid.pagePageid.id)!}');" class="btn btn-success btn-sm">首页显示</button>
															 <#else>
															<button type="button" id="isindexshow${(foodlist.id)!}" style="width:80px;"
														    onclick="updateIsindexshow('${(foodlist.id)!}','${(foodlist.productProductid.pagePageid.id)!}');" class="btn btn-success btn-sm">首页不显示</button>
														</#if>
										            </div>
									            </td>
									        <#elseif admin.username??&&admin.username=='admin' && costIdo?has_content> 
								  ...
								  <ul class="nav navbar-nav">
										...
										<li class="dropdown">
											...
											<ul class="dropdown-menu">
												<li>
													<a href="${ctx!}/admin/food/update.do?id=${(foodlist.id)!}">
									              	    修改美食
									                </a>
												</li>
												<li>
													<a href="${ctx!}/admin/food/delete.do?foodId=${(foodlist.id)!}&productId=${(foodlist.productProductid.id)!}&pageId=${(foodlist.productProductid.pageid)!}" onClick="return confirm('确定要删除?');">
									              	   删除美食
									                </a>
												</li>
												<li class="divider"></li>
												<li>
													<a href="${ctx!}/admin/price/list.do?productid=${(foodlist.productid)!}">
									              	   价格管理
									                </a>
												</li>
												<li class="divider"></li>
												...
												<li>
													<a href="${ctx!}/admin/food/view.do?id=${(foodlist.id)!}" target="_black">
									              	   美食预览
									                </a>
												</li>
