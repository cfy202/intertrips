 ...
 // 获取Cookie
  ...
  // 添加Cookie
	...
	var clearAllParam = function(){
		...
		addCookie('HpageNow',null,{path:"/"});
		addCookie('HpageSize',null,{path:"/"});
		addCookie('Hsearch',null,{path:"/"});
		addCookie('hotelIndexShow',null,{path:"/"});
		addCookie('hotelIsshow',null,{path:"/"});
		addCookie('hotelIshot',null,{path:"/"});
		$("#searchForm").find(".searchInput").val('');
		$("#pageNowInput").val('');
		window.location.href="${ctx!}/admin/hotel/list.do";
	}
	//清除cookie中存在的销售中心
	...
	function clearIsShow(status){
	    if(status==1){
	        ...
	        window.location.href="${ctx!}/admin/hotel/list.do?isshow=1";
	    } else if(status==2){
	        addCookie('tourIndexShow',null,{path:"/"});
	     	addCookie('tourIsshow',null,{path:"/"});
	        window.location.href="${ctx!}/admin/hotel/list.do?ishot=1";
	    } else if(status==3){
	        addCookie('tourIshot',null,{path:"/"});  
	        addCookie('tourIsshow',null,{path:"/"});
	        window.location.href="${ctx!}/admin/hotel/list.do?indexShow=1";
	    } else{
		    addCookie('tourIndexShow',null,{path:"/"});
			addCookie('tourIsshow',null,{path:"/"});
			addCookie('tourIshot',null,{path:"/"});
...
<script type="text/javascript">
		...
		function goPage(pageNow){
		    window.location.href="${ctx!}/admin/hotel/list.do?HpageNow="+pageNow;
		}
</script>
</head>
		...
		<div class="main-content">
			...
			<div class="page-title">
				...
				<div class="breadcrumb-env">
					<ol class="breadcrumb bc-1">
						...
						<li>
							<a href="${ctx!}/admin/hotel/list.do">酒店管理</a>
						</li>
						<li class="active">
							<strong>酒店列表</strong>
						</li>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<button class="btn btn-turquoise" onclick="window.location.href='${ctx!}/admin/hotel/add.do'">添加酒店</button>
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-2">
					<form class="navbar-form navbar-left" action="${ctx!}/admin/hotel/list.do" method="POST" role="search">
						<div class="form-group">
							<input type="text"  class="form-control" onFocus="realse()" placeholder="酒店名称、编号、城市" id="tsearch" value="${Hsearch!}" name="Hsearch">
					...
					<ul class="nav navbar-nav navbar-left">
						<li class="dropdown">
							...
							<ul class="dropdown-menu dropdown-primary">
							  ...
							  <#list costlists as costlists>
							    <li>
									<a href="${ctx!}/admin/hotel/list.do?costId=${(costlists.id)!}">${(costlists.name)!}</a>
								</li>
							  </#list>
							</ul>
						</li>
					...
					<ul class="nav navbar-nav navbar-right">
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu dropdown-primary">
							   <li>
									<a href="${ctx!}/admin/hotel/list.do?pageSize=10">10</a>
								</li>
							    <li>
									<a href="${ctx!}/admin/hotel/list.do?pageSize=20">20</a>
								</li>
							    <li>
									<a href="${ctx!}/admin/hotel/list.do?pageSize=50">50</a>
								</li>
								<li>
									<a href="${ctx!}/admin/hotel/list.do?pageSize=100">100</a>
								</li>
								<li>
									<a href="${ctx!}/admin/hotel/list.do?pageSize=${(page.totalCount)?c}">全部</a>
								</li>
							</ul>
						</li>
					</ul>
			...
			<div class="panel-body">
					...
					<form role="forl" action="${ctx!}/admin/visa/create.do" id="myfrom" method="post">
						<table id="example-3" class="table table-striped table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
							  <#list hotellist as hotellist>
							   <tr>
							       <#if costNow??&&costNow=costIdo>
							         <th class="no-sorting">
								      <#if (isCreate?? && isCreate?index_of("${(hotellist.id)!}")!=-1)>
								        <input type="checkbox" name = "tids" value="${(hotellist.id)!}" class="cbr cbr-success">
								        <#else> 
								        <input type="checkbox" name = "tids" value="${(hotellist.id)!}" class="cbr cbr-red">
								      </#if>
									...
									<#else>	
									 <#if admin.username??&&admin.username=='admin'>
										  ...
										  <th class="no-sorting">
										   <input type="checkbox" name = "tids" value="${(hotellist.id)!}">
										  </th>
									 </#if>
								   ...
								   <#if costNow??&&costNow=costIdo>
									...
									<#if hotellist.productTagList?has_content>
									    <#list hotellist.productTagList as tagnames>
									...
									<td>
									   <div id="gallery">
									    <#if hotellist.productProductid.imageurl?has_content>
										    <#list hotellist.productProductid.imageurl?split(",") as urls>
										     <#if urls_index=0>
									...
									<td>${(hotellist.productProductid.code)!}</td>
									<td>${(hotellist.hotelcn)!}</td>
									<td>${(hotellist.hotelstar)!}</td>
									<td>${(hotellist.city)!}</td>
								  <#if costNow??&&costNow=costIdo>
									      <td>
										    <div class="btn-group" width="270">
										              
										                  <#if (hott?? && hott?index_of("${(hotellist.id)!}")!=-1)>
														    <button type="button" id="ishot${(hotellist.id)!}" style="width:60px;"
														    onclick="updateIshot('${(hotellist.id)!}','${(hotellist.productProductid.pagePageid.id)!}');" class="btn btn-red btn-sm">热推</button>
															 <#else>
															<button type="button" id="ishot${(hotellist.id)!}" style="width:60px;"
														    onclick="updateIshot('${(hotellist.id)!}','${(hotellist.productProductid.pagePageid.id)!}');" class="btn btn-red btn-sm">不热推</button>
														</#if>
														 <#if (show?? && show?index_of("${(hotellist.id)!}")!=-1)>
														    <button type="button" id="isshow${(hotellist.id)!}" style="width:60px;"
														    onclick="updateIsshow('${(hotellist.id)!}','${(hotellist.productProductid.pagePageid.id)!}');" class="btn btn-blue btn-sm">显示</button>
															 <#else>
															<button type="button" id="isshow${(hotellist.id)!}" style="width:60px;"
														    onclick="updateIsshow('${(hotellist.id)!}','${(hotellist.productProductid.pagePageid.id)!}');" class="btn btn-blue btn-sm">不显示</button>
														</#if>
														 <#if (index?? && index?index_of("${(hotellist.id)!}")!=-1)>
														    <button type="button" id="isindexshow${(hotellist.id)!}" style="width:80px;"
														    onclick="updateIsindexshow('${(hotellist.id)!}','${(hotellist.productProductid.pagePageid.id)!}');" class="btn btn-success btn-sm">首页显示</button>
															 <#else>
															<button type="button" id="isindexshow${(hotellist.id)!}" style="width:80px;"
														    onclick="updateIsindexshow('${(hotellist.id)!}','${(hotellist.productProductid.pagePageid.id)!}');" class="btn btn-success btn-sm">首页不显示</button>
														</#if>
										            </div>
									            </td>
									        <#elseif admin.username??&&admin.username=='admin' && costIdo?has_content> 
								  ...
								  <ul class="nav navbar-nav">
										...
										<li class="dropdown">
											...
											<ul class="dropdown-menu">
												<li>
													<a href="${ctx!}/admin/hotel/update.do?id=${(hotellist.id)!}">
									              	    修改酒店
									                </a>
												</li>
												<li>
													<a href="${ctx!}/admin/hotel/delete.do?hotelId=${(hotellist.id)!}&productId=${(hotellist.productProductid.id)!}&pageId=${(hotellist.productProductid.pageid)!}" onClick="return confirm('确定要删除?');">
									              	   删除酒店
									                </a>
												</li>
												<li class="divider"></li>
												<li>
													<a href="${ctx!}/admin/price/list.do?productid=${(hotellist.productid)!}">
									              	   价格管理
									                </a>
												</li>
												<li class="divider"></li>
												...
												<li>
													<a href="${ctx!}/admin/hotel/view.do?id=${(hotellist.id)!}" target="_black">
									              	   酒店预览
									                </a>
												</li>
