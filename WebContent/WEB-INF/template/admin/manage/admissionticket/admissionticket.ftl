...
<head>
 ...
 // 获取Cookie
  ...
  // 添加Cookie
    ...
    function realse(){
      addCookie("Hsearch",null,{path:"/"});
    }
    
    //清除所有参数
	var clearAllParam = function(){
	    addCookie('costId',null,{path:"/"});
		addCookie('AtpageNow',null,{path:"/"});
		addCookie('AtpageSize',null,{path:"/"});
		addCookie('Atsearch',null,{path:"/"});
		addCookie('admissionticketIndexShow',null,{path:"/"});
		addCookie('admissionticketIsshow',null,{path:"/"});
		addCookie('admissionticketIshot',null,{path:"/"});
		$("#searchForm").find(".searchInput").val('');
		$("#pageNowInput").val('');
		window.location.href="${ctx!}/admin/admissionticket/list.do";
	}
	//清除cookie中存在的销售中心
	...
	function clearIsShow(status){
	    if(status==1){
	        ...
	        window.location.href="${ctx!}/admin/admissionticket/list.do?isshow=1";
	    } else if(status==2){
	        addCookie('tourIndexShow',null,{path:"/"});
	     	addCookie('tourIsshow',null,{path:"/"});
	        window.location.href="${ctx!}/admin/admissionticket/list.do?ishot=1";
	    } else if(status==3){
	        addCookie('tourIshot',null,{path:"/"});  
	        addCookie('tourIsshow',null,{path:"/"});
	        window.location.href="${ctx!}/admin/admissionticket/list.do?indexShow=1";
	    } else{
		    addCookie('tourIndexShow',null,{path:"/"});
			addCookie('tourIsshow',null,{path:"/"});
			addCookie('tourIshot',null,{path:"/"});
...
<script type="text/javascript">
		...
		function goPage(pageNow){
		    window.location.href="${ctx!}/admin/admissionticket/list.do?FopageNow="+pageNow;
		}
</script>
</head>
		...
		<div class="main-content">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<button class="btn btn-turquoise" onclick="window.location.href='${ctx!}/admin/admissionticket/add.do'">添加门票</button>
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-2">
					<form class="navbar-form navbar-left" action="${ctx!}/admin/admissionticket/list.do" method="POST" role="search">
						<div class="form-group">
							<input type="text"  class="form-control" onFocus="realse()" placeholder="门票名称、编号、城市" id="tsearch" value="${Atsearch!}" name="Atsearch">
						    <input type="hidden" name ="HpageNow" value="${(page.pageNow)!}">
					...
					<ul class="nav navbar-nav navbar-left">
						<li class="dropdown">
							...
							<ul class="dropdown-menu dropdown-primary">
							  ...
							  <#list costlists as costlists>
							    <li>
									<a href="${ctx!}/admin/admissionticket/list.do?costId=${(costlists.id)!}">${(costlists.name)!}</a>
								</li>
							  </#list>
							</ul>
						</li>
					...
					<ul class="nav navbar-nav navbar-right">
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu dropdown-primary">
							   <li>
									<a href="${ctx!}/admin/admissionticket/list.do?pageSize=10">10</a>
								</li>
							    <li>
									<a href="${ctx!}/admin/admissionticket/list.do?pageSize=20">20</a>
								</li>
							    <li>
									<a href="${ctx!}/admin/admissionticket/list.do?pageSize=50">50</a>
								</li>
								<li>
									<a href="${ctx!}/admin/admissionticket/list.do?pageSize=100">100</a>
								</li>
								<li>
									<a href="${ctx!}/admin/admissionticket/list.do?pageSize=${(page.totalCount)?c}">全部</a>
								</li>
							</ul>
						</li>
					</ul>
			...
			<div class="panel-body">
					...
					<form role="forl" action="${ctx!}/admin/visa/create.do" id="myfrom" method="post">
						<table id="example-3" class="table table-striped table-bordered" cellspacing="0" width="100%">
							...
							<tbody>
							  <#list admissionlist as admissionlist>
							   <tr>
							       <#if costNow??&&costNow=costIdo>
							         <th class="no-sorting">
								      <#if (isCreate?? && isCreate?index_of("${(admissionlist.id)!}")!=-1)>
								        <input type="checkbox" name = "tids" value="${(admissionlist.id)!}" class="cbr cbr-success">
								        <#else> 
								        <input type="checkbox" name = "tids" value="${(admissionlist.id)!}" class="cbr cbr-red">
								      </#if>
									...
									<#else>	
									 <#if admin.username??&&admin.username=='admin'>
										  ...
										  <th class="no-sorting">
										   <input type="checkbox" name = "tids" value="${(admissionlist.id)!}">
										  </th>
									 </#if>
								   ...
								   <#if costNow??&&costNow=costIdo>
									...
									<#if admissionlist.productTagList?has_content>
									    <#list admissionlist.productTagList as tagnames>
									...
									<td>
									   <div id="gallery">
									    <#if admissionlist.productProductid.imageurl?has_content>
										    <#list admissionlist.productProductid.imageurl?split(",") as urls>
										     <#if urls_index=0>
									...
									<td>${(admissionlist.ticketname)!}</td>
									<td>${(admissionlist.productProductid.code)!}</td>
									<td>${(admissionlist.productProductid.minprice)!}</td>
									<td>${(admissionlist.ticketcn)!}</td>
									<td>${(admissionlist.city)!}</td>
								  <#if costNow??&&costNow=costIdo>
									      <td>
										    <div class="btn-group" width="270">
										              
										                  <#if (hott?? && hott?index_of("${(admissionlist.id)!}")!=-1)>
														    <button type="button" id="ishot${(admissionlist.id)!}" style="width:60px;"
														    onclick="updateIshot('${(admissionlist.id)!}','${(admissionlist.productProductid.pagePageid.id)!}');" class="btn btn-red btn-sm">热推</button>
															 <#else>
															<button type="button" id="ishot${(admissionlist.id)!}" style="width:60px;"
														    onclick="updateIshot('${(admissionlist.id)!}','${(admissionlist.productProductid.pagePageid.id)!}');" class="btn btn-red btn-sm">不热推</button>
														</#if>
														 <#if (show?? && show?index_of("${(admissionlist.id)!}")!=-1)>
														    <button type="button" id="isshow${(admissionlist.id)!}" style="width:60px;"
														    onclick="updateIsshow('${(admissionlist.id)!}','${(admissionlist.productProductid.pagePageid.id)!}');" class="btn btn-blue btn-sm">显示</button>
															 <#else>
															<button type="button" id="isshow${(admissionlist.id)!}" style="width:60px;"
														    onclick="updateIsshow('${(admissionlist.id)!}','${(admissionlist.productProductid.pagePageid.id)!}');" class="btn btn-blue btn-sm">不显示</button>
														</#if>
														 <#if (index?? && index?index_of("${(admissionlist.id)!}")!=-1)>
														    <button type="button" id="isindexshow${(admissionlist.id)!}" style="width:80px;"
														    onclick="updateIsindexshow('${(admissionlist.id)!}','${(admissionlist.productProductid.pagePageid.id)!}');" class="btn btn-success btn-sm">首页显示</button>
															 <#else>
															<button type="button" id="isindexshow${(admissionlist.id)!}" style="width:80px;"
														    onclick="updateIsindexshow('${(admissionlist.id)!}','${(admissionlist.productProductid.pagePageid.id)!}');" class="btn btn-success btn-sm">首页不显示</button>
														</#if>
										            </div>
									            </td>
									        <#elseif admin.username??&&admin.username=='admin' && costIdo?has_content> 
								  ...
								  <ul class="nav navbar-nav">
										...
										<li class="dropdown">
											...
											<ul class="dropdown-menu">
												<li>
													<a href="${ctx!}/admin/admissionticket/update.do?id=${(admissionlist.id)!}">
									              	    修改门票
									                </a>
												</li>
												<li>
													<a href="${ctx!}/admin/admissionticket/delete.do?admissionId=${(admissionlist.id)!}&productId=${(admissionlist.productProductid.id)!}&pageId=${(admissionlist.productProductid.pageid)!}" onClick="return confirm('确定要删除?');">
									              	   删除门票
									                </a>
												</li>
												<li class="divider"></li>
												<li>
													<a href="${ctx!}/admin/price/list.do?productid=${(admissionlist.productid)!}">
									              	   价格管理
									                </a>
												</li>
												<li class="divider"></li>
												...
												<li>
													<a href="${ctx!}/admin/admissionticket/view.do?id=${(admissionlist.id)!}" target="_black">
									              	   门票预览
									                </a>
												</li>
